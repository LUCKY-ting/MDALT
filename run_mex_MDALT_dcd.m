...
for run = 1:times
    index = randperm(n);
    w = MD_ALT_train_dcd_sparse(trainData',trainLabel', index, miu, theta, eta, maxIterNum, epoch);
    tic
...
fprintf(fid,'name = bibtex, linear MDALT_DCD, runTimes = %d, epoch = %d, maxIter = %d \n', times, epoch, maxIterNum);
